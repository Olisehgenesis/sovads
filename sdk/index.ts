...
export function renderAttachedCtas(opts: {
  ...
  if (overlay) panel.setAttribute('data-overlay', '1')
...
function buildTaskRow(
  ...
  ctx: {
    ...
    /** Row is being rendered inside an overlay panel that sits on top of the
     *  banner image (POLL / QUIZ). Tiles still capture clicks; the surrounding
     *  panel is pointer-events:none so the banner stays clickable. */
    overlay?: boolean
  }
): HTMLElement {
  ...
  if (task.kind === 'VISIT_URL') {
    ...
    const labelEl = btn.firstElementChild as HTMLElement | null
    const setLabel = (text: string) => {
      if (labelEl && labelEl.tagName === 'SPAN') labelEl.textContent = text
    ...
    const renderSaved = (text: string, tone: 'ok' | 'err') => {
    ...
    optionsList.slice(0, 5).forEach((opt, idx) => {
      ...
      if (n === 5 && idx === 4) tile.style.gridColumn = '1 / -1'
      ...
      wireClick(tile, async () => {
        ...
        for (const child of Array.from(optionsWrap.children)) {
          ...
          ;(child as HTMLButtonElement).style.cursor = 'pointer'
        }
        tile.style.outline = ''
        setStatus(result.error || `Submit failed (${result.status})`, 'err')
      })

      applyDoneStyling(tile as unknown as HTMLButtonElement)
    ...
    `
      ...
      if (
        ...
        if (allChoice) {
          ...
          if (!pos || pos === 'static') adElement.style.position = 'relative'
        }
        try {
          renderAttachedCtas({
